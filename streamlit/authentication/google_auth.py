...
def decode_user(token:str):
    ...
    return: decoded user data including email and name
    """
...
def show_login_button(text: Optional[str] = "Login with Google",
                      color="#FD504D",
                      sidebar: bool = True):
...
def show_logout_button(text: Optional[str] = "Account Logout",
                        color = "#FD504D",
                        sidebar: bool = True):
    if "email" in st.session_state:
        del st.session_state["email"]
        markdown_button(redirect_url, text="Logout")